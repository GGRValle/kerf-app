...
**You:** OpenAI Codex, working in pair with Claude Code on this repo.
**Companion file:** `kerf-app/CLAUDE.md` — Claude Code's project file. Read both. The two files agree on architecture; they differ only on per-agent role guidance.

> Read your global `~/.codex/CODEX.md` first. It defers to `~/.claude/CLAUDE.md` as the cross-project source of truth. Everything in this file is project-scoped to `kerf-app/`.
...
## 2. Operating mode — pair with Claude Code

You and Claude Code work the same repo, alternating author and reviewer roles per feature. The split is in the master doc §11.2.
...
**When you author:** describe what could break. Make it easy for Claude Code to find the bugs you missed.
...
**When you and Claude Code disagree:** post your reasoning, let Claude Code respond, and wait for Christian to arbitrate. The architecture principle that supports the decision becomes a precedent logged in this file or `CLAUDE.md`.

---

## 3. Architecture invariants — DO NOT VIOLATE

These come from `README.md`, the global `CLAUDE.md`, and existing code. Every PR must respect them.
...
## 4. The 12 V1 schema additions (W0 — your shared work with Claude Code)
...
|---|---|---|---|
| 1 | Authority profile | Claude Code | **You** |
| 2 | Decision altitude class (`L0`–`L4`) | **You** | Claude Code |
| 3 | Privilege class tag + LLM bypass | Claude Code | **You** |
| 4 | Read audit log | **You** | Claude Code |
| 5 | i18n verification (already in code) | n/a | n/a |
| 6 | Tenant key wrapper | Claude Code | **You** |
| 7 | Client-share entity kinds (`mood_board`, `client_share`, `design_revision`, `client_decision`) | **You** | Claude Code |
| 8 | Compliance KB stub | **You** | Claude Code |
| 9 | User-privacy data class + retention policy | Claude Code | **You** |
| 10 | Automation guardrails (`automation`, `automation_run`, `guardrail_trip`) | **You** | Claude Code |
| 11 | Usage tier subscription (`tenant_subscription`, `usage_event`) | **You** | Claude Code |
| 12 | Cost KB entry kind (`cost_kb_entry`, `cost_override`) | Claude Code | **You** |

**Recommended first PR (calibration, before #1–12):** add a missing test on `permissions/matrix.ts` for the owner money ceiling. Small, scoped, validates the workflow.

**Then start with #2 (decision altitude class)** — pure additive type work, no logic changes, easiest to land cleanly. Build the workflow muscle on small wins before the heavier additions.
...
2. **Check `kerf-app/CLAUDE.md`** — Claude Code's project file; same architecture rules
3. **Check `~/.claude/CLAUDE.md`** — global file, single source of truth across projects
4. **Check the master doc** — `_docs/investor/Kerf_Master_Build_Capital_Direction_v2_1.docx` for strategic context, schema commitments, and the parallel-build protocol
5. **Check the Build Pivot Spec** — `_docs/product/Kerf_Build_Pivot_Spec_Apr23.md`
6. **Open a PR with `[QUESTION]` prefix** — flag the uncertainty in the description, propose two paths, let Claude Code or Christian decide
7. **Do not guess.** Source-or-silent. If the answer isn't in writing, surface the question.
...
## 9. Project state (Apr 26, 2026)

- W0 prerequisites: git init complete, GitHub remote established
- Next: 12 schema additions over W0 (~5.5 days)
- Then: W1 invoice follow-up workflow (Apr 28 – May 4)
...
- Apr 26, 2026 — Initial draft, post-positioning-v3 lock. Christian to review before W0 #1 PR.
