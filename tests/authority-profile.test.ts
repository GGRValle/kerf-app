...
test('approve_under_ceiling without amountCents allows (no money in play)', () => {
  // PM is approve_under_ceiling on money. If no amount supplied, the band
  // should resolve as 'allowed' (the ceiling is not engaged).
  assert.equal(
    canAuthorize(DEFAULT_AUTHORITY_PROFILE, {
      role: 'pm',
      resource: 'money',
