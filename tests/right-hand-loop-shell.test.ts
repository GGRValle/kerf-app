...
test('owner home leads with route-anywhere, not a duplicated domain chip row', () => {
  ...
  assert.match(source, /uniqueByContent/);
  assert.match(source, /onDeckLimit: 3/);
  assert.match(source, /pulseLimit: 4/);
  assert.doesNotMatch(source, /loop-strip/);
});
