...
**F&F / proposal-first framing:** the interactive queue demo is written so
**proposal follow-up** is the lead path (review panel, drafted client
follow-up, operator actions). **Invoice** and **drift** cards are supporting
loops in the same queue — they prove the surface generalizes without pulling
attention away from the proposal story.
...
- Top bar: `KERF`, `W1 · Proposal review first`, tag line calling out invoice
  and drift as supporting loops, optional lead blurb under the title, and
  `Local only`.
- Main queue with 12 cards from `seededMixedDecisionPacketListFixture`
  ...
  Proposals sort first in **All**; rehearse proposal review before invoice/drift
  filters when showing F&F.
...
For F&F, walk **proposal** first (tinted cards + review panel), then use
invoice/drift filters to show supporting loops.
