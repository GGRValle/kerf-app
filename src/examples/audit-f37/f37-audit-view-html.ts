...
function renderTranscriptSection(): string {
  return `
  <section class="kerf-f37__section" aria-labelledby="f37-transcript-h">
    ...
    <pre class="kerf-f37__mono" aria-label="Original transcript">${escapeHtml(TRANSCRIPT_ORIGINAL)}</pre>
    <p><strong>Edits (overlay events)</strong> — each edit is an append-only correction record.</p>
    <pre class="kerf-f37__mono" aria-label="Transcript overlay">${escapeHtml(TRANSCRIPT_EDIT_SNIPPET)}</pre>
    <p><strong>Current view</strong> — rendered for operators as original + ordered overlays (demo text below).</p>
    <pre class="kerf-f37__mono" aria-label="Rendered current transcript">${escapeHtml(transcriptCurrentDisplay())}</pre>
  </section>`;
}
...
export function buildF37AuditPageHtml(packet: DecisionPacket, selectedId: string, variant: F37AuditVariant): string {
  ...
  const top =
    ...
    <div class="kerf-f37__meta">F-37 · Audit log / event stream · local demo</div>
      ...
      : `<p class="kerf-f37-embed-lede kerf-v15-prose" role="note"><strong>Audit stream demo.</strong> Read-only timeline from the F-37 fixture on this DecisionPacket (no writes; no separate server).</p>`;
  ...
  ${renderValidators(packet)}
  ${renderBlackboardPreview(packet)}
